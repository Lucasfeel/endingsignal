(() => {
  ...
  const renderSelectedContent = () => {
    ...
    const thumbnail = item.thumbnail_url || item.thumbnail || item.thumbnail_path;
  ...
  const renderDeletedList = () => {
    ...
    STATE.deleted.items.forEach((item) => {
      ...
      const meta = document.createElement('div');
      meta.className = 'mt-1 text-xs text-white/60';
      meta.textContent = `${item.source || '-'} · ${item.content_type || '-'}`;
  ...
  const renderPublications = () => {
    ...
    STATE.publications.items.forEach((item) => {
      ...
      const meta = document.createElement('div');
      meta.className = 'mt-1 text-xs text-white/60';
      meta.textContent = `${item.source || '-'} · 공개일: ${formatTimestamp(item.public_at)} · 사유: ${item.reason || '-'}`;
  ...
  const bindEvents = () => {
    ...
    manageSearchBtn?.addEventListener('click', performSearch);
    ...
    document.getElementById('publicationSaveBtn')?.addEventListener('click', savePublication);
    ...
    document.getElementById('publicationsPrevBtn')?.addEventListener('click', () => {
    ...
    document.getElementById('publicationsNextBtn')?.addEventListener('click', () => {

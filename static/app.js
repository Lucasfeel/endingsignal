...
function createCard(content, tabId, aspectClass) {
  ...
  const kakaoChar = kakaoAssets?.c2 || kakaoAssets?.c1;
  const kakaoTitle = kakaoAssets?.t2 || kakaoAssets?.t1;
  ...
  if (isKakao) {
    ...
    setClasses(stack, UI_CLASSES.thumbStack);
    ...
    if (kakaoChar) {
      ...
      charEl.src = kakaoChar;
      charEl.onerror = () => {
        charEl.style.display = 'none';
      };
      setClasses(charEl, UI_CLASSES.thumbChar);
      stack.appendChild(charEl);
    }

...
const setAccessToken = (token) => {
  try {
    if (token) {
      localStorage.setItem('es_access_token', token);
    }
  } catch (e) {
    console.warn('Failed to write access token', e);
  }
};
...
const requireAuthOrPrompt = (actionName) => {
  const token = getAccessToken();
  if (!token) {
    showToast('로그인이 필요합니다.', { type: 'error' });
    openAuthModal({ reason: actionName || 'auth-required' });
    return false;
  }
  return true;
};
...
async function apiRequest(method, path, { query, body, token } = {}) {
  ...
  if (!response.ok) {
    if ((response.status === 401 || response.status === 403) && (token || getAccessToken())) {
...
function setAuthState({ isAuthenticated, user, isChecking }) {
  if (typeof isAuthenticated === 'boolean') STATE.auth.isAuthenticated = isAuthenticated;
  if (user !== undefined) STATE.auth.user = user;
  if (typeof isChecking === 'boolean') STATE.auth.isChecking = isChecking;
  updateProfileButton();
}

function handleUnauthorized(message) {
  logout({ silent: true, skipRender: true });
  if (message) {
    showToast(message, { type: 'error' });
  }
  openAuthModal({ reason: 'unauthorized' });
  if (STATE.activeTab) {
...
async function fetchMe() {
  const token = getAccessToken();
  if (!token) {
    setAuthState({ isAuthenticated: false, user: null, isChecking: false });
    return null;
  }

  setAuthState({ isChecking: true });
  try {
...
async function login(email, password) {
  ...
  const token =
    res?.data?.access_token || res?.access_token || res?.data?.token || res?.token;
  if (!token) {
...
function logout({ silent = false, skipRender = false } = {}) {
  clearAccessToken();
...
async function unsubscribeContent(content) {
  const token = getAccessToken();
  if (!token) {
    requireAuthOrPrompt('unsubscribe');
    return;
  }
...
function updateProfileButton() {
  if (!UI.profileButton) return;
  const btn = UI.profileButton;
  if (STATE.auth.isAuthenticated) {
    const email = STATE.auth.user?.email || '';
    const initial = email ? email.charAt(0).toUpperCase() : '🟣';
    btn.textContent = initial;
    btn.classList.remove('bg-[#525252]', 'text-white/80');
    btn.classList.add('bg-[#4f46e5]', 'text-white');
  } else {
    btn.textContent = 'IN';
    btn.classList.remove('bg-[#4f46e5]', 'text-white');
    btn.classList.add('bg-[#525252]', 'text-white/80');
  }
}
...
async function handleAuthSubmit(evt) {
  ...
  if (!emailValid) {
    if (UI.authError) UI.authError.textContent = '이메일 형식을 확인해주세요.';
    return;
  }
  if (password.length < 6) {
    if (UI.authError) UI.authError.textContent = '비밀번호는 6자 이상이어야 합니다.';
    return;
  }
...
function setupAuthUI() {
  ...
  if (UI.authCancelButton) UI.authCancelButton.onclick = () => closeAuthModal();
  if (UI.authSubmitButton) UI.authSubmitButton.onclick = (e) => handleAuthSubmit(e);
  if (UI.authPassword) {
